...
function puppet_master_admin_main()
{
	...
	// if there are pms
	if(is_array($puppet_masters) && !empty($puppet_masters))
	{
		// list them
...
function puppet_master_admin_edit()
{
	...
	if(isset($mybb->input['uid']) && $mybb->input['uid'])
	{
		...
		if(is_array($puppets) && !empty($puppets))
		{
...
/*
 * puppet_master_admin_permissions()
 *
 ...
 * output the tabs
 *
 * @param - $current - (string) the current page key
 * @return: n/a
 */
